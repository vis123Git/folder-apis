...
exports.create_folder = async (req, res) => {
  try {
    ...
    const check_one_folder = await find_folders({ user_id }, { user_id: 1, name: 1, parent_folder: 1 });
    if (check_one_folder && check_one_folder.length) {
      ...
      const parent_exists = await find_one_folder({ _id: parent_folder , user_id }, {});      
      if (!parent_exists) return res.status(400).json({ status: false, message: "Parent folder does not exists!" });

      const folder_exists = await find_one_folder({ name: folder_name, parent_folder: parent_folder, user_id }, {});
      if (folder_exists) return res.status(400).json({ status: false, message: "Folder already exists!" });
    }


    const add_folder = await add_new_folder({ name: folder_name, parent_folder: check_one_folder.length ? parent_folder : null, user_id });
...
exports.get_one_folder = async (req, res) => {
  try {
    const user_id = req.user_id;
    const { id } = req.params;
    ...
    return res.status(201).json({ status: true,data:folder_exists, message: "Folder found successfully" });
  } catch (error) {
    console.log("error===>", error);
    return res.status(400).json({ status: false, message: "Something went wrong!" });
  }
